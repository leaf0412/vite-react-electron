...
const StartupAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleLoadingProgress: IpcRendererEventCallback<[number]> = (
      ...
      console.log('handleLoadingProgress', progress);
      const validProgress = Math.max(0, Math.min(1, progress));
      progressRef.current = validProgress;
    };

    window.ipcRenderer?.startupLoadingProgress('on', handleLoadingProgress);

    return () => {
      window.removeEventListener('resize', handleResize);
      window.ipcRenderer?.startupLoadingProgress('off', handleLoadingProgress);
